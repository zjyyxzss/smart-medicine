...
// (或叫 Medicine)
import world.xuewei.dao.MedicineRepository;
...
public class MedicineServiceImpl extends BaseService<Medicine> implements MedicineService {
    ...
    @Autowired
    private MedicineRepository medicineRepository; // 注入 ES Repository

    @Autowired
    private ElasticsearchOperations elasticsearchTemplate; // 注入 ES 操作模板
    ...
    // 6. 【核心】实现 Day 7 的两个接口方法

    /**
     * Day 7 核心：将 MySQL 数据全量同步到 Elasticsearch
     */
    @Override
    public RespResult syncDrugsToEs() {
        List<Medicine> mysqlDrugs = medicineDao.selectList(null);
        if (mysqlDrugs.isEmpty()) {
            return RespResult.fail("MySQL 中没有药品数据");
        }
        List<MedicineDocument> esDocuments = mysqlDrugs.stream()
                .map(MedicineDocument::new)
                .collect(Collectors.toList());
        medicineRepository.saveAll(esDocuments);
        return RespResult.success("成功同步 " + esDocuments.size() + " 条药品数据到 ES");
    }

    /**
     * Day 7 核心：实现 ES 搜索逻辑 (使用 Spring Boot 3.x 新版 Query DSL)
     */
    @Override
    public List<MedicineDocument> searchDrugs(String keyword) {
        ...
        SearchHits<MedicineDocument> searchHits = elasticsearchTemplate.search(query, MedicineDocument.class);
